    ...
    public:
        ...
        ProxyEntity entity_to_proxy(const std::shared_ptr<Entity>& entity, double world_time) {
          ...
          //  UtilityFunctions::print("Created proxy with last sim time ", world_time);
            ...
            if (auto colonist = std::dynamic_pointer_cast<Colonist>(entity)) {
                ...
                proxy.entity_type = colonist->get_type_id();
                ...
                proxy.entity_sprite = building->get_entity_sprite();
                proxy.entity_type =building->get_type_id();
                proxy.entity_size =building->get_entity_size();
                proxy.base_move_speed =building->get_base_move_speed();
                proxy.must_simulate =building->is_must_simulate();

                // Copy the entire job list
                proxy.job_list =building->get_job_list();
                proxy.current_job_index =building->get_current_job_index();
                // If current job index is invalid, find highest priority job
                if (proxy.current_job_index == -1 || 
                        proxy.current_job_index >= proxy.job_list.size() ||
                        proxy.job_list[proxy.current_job_index].complete) {
        ...
        std::shared_ptr<Entity> proxy_to_entity(const ProxyEntity& proxy, double current_time) {
            ...
            else if(proxy.entity_type == 2)
            {
                ...
                // Construct with proper parameters
                auto entity = std::make_shared<Building>(
                        extrapolated_pos,
                        proxy.entity_id,
                        proxy.entity_sprite,
                        proxy.entity_size
            ...
            {
                UtilityFunctions::printerr("NULLPTR ENTITY CREATED FROM PROXY - UNKNOWN TYPE");
                return nullptr;
            }
